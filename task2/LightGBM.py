...
CONFIG = {
    ...
    "lgb_model_path": 'lgb_classifier_model.pkl',
...
if os.path.exists(CONFIG["lgb_model_path"]):
    print("Loading pre-trained LightGBM model...")
    lgb_model = joblib.load(CONFIG["lgb_model_path"])
else:
    print("Training LightGBM model...")
    lgb_model = lgb.LGBMClassifier()
    lgb_model.fit(train_features, np.argmax(train_labels, axis=1))
...
print(f"LightGBM Model Accuracy: {accuracy:.2%}")
