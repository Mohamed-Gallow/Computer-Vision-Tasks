...
CONFIG = {
    ...
    "dt_model_path": 'dt_classifier_model.pkl',
...
if os.path.exists(CONFIG["dt_model_path"]):
    ...
    dt_classifier = joblib.load(CONFIG["dt_model_path"])
else:
    print("Training Decision Tree classifier model...")
    dt_classifier = DecisionTreeClassifier()
    dt_classifier.fit(train_features, np.argmax(train_labels, axis=1))
    joblib.dump(dt_classifier, CONFIG["dt_model_path"])
    print("Decision Tree classifier model saved.")

# Evaluate Decision Tree Classifier
val_predictions = dt_classifier.predict(val_features)
