...
CONFIG = {
    ...
    "nb_model_path": 'nb_classifier_model.pkl',
...
# Train or Load Gaussian Naive Bayes Model
if os.path.exists(CONFIG["nb_model_path"]):
    print("Loading pre-trained Gaussian Naive Bayes model...")
    nb_model = joblib.load(CONFIG["nb_model_path"])
else:
    print("Training Gaussian Naive Bayes model...")
    nb_model = GaussianNB()
    nb_model.fit(train_features, np.argmax(train_labels, axis=1))
    joblib.dump(nb_model, CONFIG["nb_model_path"])
...
print(f"Gaussian Naive Bayes Model Accuracy: {accuracy:.2%}")
